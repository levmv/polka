...
test('PDF reader behavior', async ({ page, browserName, browserErrors }) => {
  ...
  const file = pdf(title, 'PDF Fixture Author', 'pdf-reader');
  ...
  await test.step('replaces searches and highlights the selected text', async () => {
    ...
    // Escape belongs to the open search panel even when focus is on the page.
    await stage.focus();
...
test('PDF highlights retain their page and geometry through zoom, navigation and reopening', async ({
  page,
}) => {
  test.setTimeout(30_000);
  const bookId = await importTestBook(
    page,
    pdf('PDF Highlights', 'PDF Author', 'pdf-highlights', [0, 90], 'Another line'),
  );
  await page.goto(`/read/${bookId}`);
  ...
  for (const [index, label] of ['First PDF page', 'Second PDF page'].entries()) {
    ...
    await page
      ...
      .getByRole('button', { name: index === 0 ? 'Add note' : 'Highlight', exact: true })
      .click();
    if (index === 0) {
  ...
  await expect(page.locator('[data-reader-progress-text]')).toBeVisible();
  await expect(page.locator('.book-annotations-count')).toHaveText('2');
