...
test.use({ account: 'reader', timezoneId: 'Asia/Tokyo' });
...
test.describe('Reading activity', () => {
  test('initializes the reader’s own zone and preserves an explicit choice', async ({ page }) => {
    ...
    await page.goto('/');
    await expect
      .poll(async () => (await (await page.request.get('/api/settings')).json()).time_zone)
      .toBe('Asia/Tokyo');
  ...
  test('bounds an abandoned reader and resumes only after a real action', async ({ page }) => {
    ...
    await page.route('**/api/reader/assets/*/activity', async (route) => {
      ...
      await route.fulfill({
        json: { active: !payload.finished, counted_ms: payload.elapsed_ms || 0 },
      });
    });
    ...
    const resumed = page.waitForResponse(
      (r) => r.url().endsWith('/activity') && r.request().method() === 'POST',
    );
    ...
    await expect
      .poll(() => checkpoints.some((p) => p.session_id === starts[1] && !p.finished))
      .toBe(true);
  });
  ...
  test('closing a reader persists its final checkpoint on the server', async ({ page }) => {
    ...
    await expect
      ...
      .toBe(false);
    const result = await (
      await page.request.post(endpoint, { data: { session_id: sessionId } })
    ).json();
  ...
  test('a quick return cannot overtake the last report before a pause', async ({ page }) => {
    ...
    try {
      ...
      const pauseSent = page.waitForRequest(
        (r) => r.url().endsWith('/activity') && r.method() === 'PUT',
      );
      await setVisibility(page, 'hidden');
      await pauseSent;
  ...
  test('a displaced reader cannot reclaim time through background checkpoints', async ({
    page,
    browserErrors,
  }) => {
    const starts: string[] = [];
    ...
    page.on('request', (request) => {
      if (
        request.method() === 'POST' &&
        /\/api\/reader\/assets\/[^/]+\/activity$/.test(request.url())
      ) {
    ...
    browserErrors.allow(
      (message) =>
        message === `Failed to load resource: net::ERR_FAILED [${endpoint}]` &&
        expectedNetworkErrors++ === 0,
    );
    ...
    const navigationSave = page.waitForEvent(
      'requestfailed',
      (r) => r.url() === endpoint && r.method() === 'PUT',
    );
    await page.keyboard.press('ArrowRight');
    await navigationSave;
    const claim = await page.request.post(endpoint, {
      data: { session_id: 'ffffffffffffffffffffffffffffffff' },
    });
    expect((await claim.json()).active).toBe(true);
    const displaced = page.waitForResponse(
