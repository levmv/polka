...
func TestTrashLifecycleHTTP(t *testing.T) {
	...
	if w := do(member.ID, http.MethodDelete, "/api/books/2"); w.Code != http.StatusNoContent {
		t.Fatalf("member delete = %d, want 204; body: %s", w.Code, w.Body)
	}

	if got := listBookIDs(t, do(member.ID, http.MethodGet, "/api/books")); len(got) != 1 || got[0] != 1 {
		t.Fatalf("browse after delete = %v, want [1]", got)
	}
	if w := do(member.ID, http.MethodGet, "/api/books/2"); w.Code != http.StatusNotFound {
	...
	if len(trash) != 1 || trash[0].ID != 2 || trash[0].DeletedBy != "member" || trash[0].DeletedAt == 0 {
		t.Fatalf("trash listing = %+v, want one 2 trashed by member", trash)
	}

	if w := do(member.ID, http.MethodPost, "/api/books/2/restore"); w.Code != http.StatusNoContent {
	...
	if w := do(member.ID, http.MethodDelete, "/api/books/1/purge"); w.Code != http.StatusForbidden {
	...
	if w := do(admin.ID, http.MethodDelete, "/api/books/1/purge"); w.Code != http.StatusNoContent {
...
func TestEmptyTrashHTTP(t *testing.T) {
	...
	for _, id := range []int64{1, 2} {
		if err := db.SoftDeleteBook(database.Write(t.Context()), id, member.ID); err != nil {
...
func TestPurgeUnavailableRootPreservesCatalog(t *testing.T) {
	tests := []struct {
		...
		{name: "single", target: "/api/books/1/purge", ids: []int64{1}},
		{name: "empty trash", target: "/api/trash", ids: []int64{1, 2}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, id := range tt.ids {
				if err := db.SoftDeleteBook(database.Write(t.Context()), id, admin.ID); err != nil {
					t.Fatalf("soft delete %d: %v", id, err)
				}
			}
			...
			for _, id := range tt.ids {
				var deleted bool
				if err := database.Read(req.Context()).QueryRow("SELECT deleted_at IS NOT NULL FROM books WHERE id = ?", id).Scan(&deleted); err != nil {
					t.Fatalf("book %d disappeared after rejected purge: %v", id, err)
				}
				if !deleted {
					t.Fatalf("book %d was restored during rejected purge", id)
				}
			}
			assetPath := filepath.Join(dir, "Tolkien", "The_Hobbit", "a_1.epub")
			if _, err := os.Stat(assetPath); err != nil {
...
func TestPurgeWaitsForStorageSlot(t *testing.T) {
	...
	if err := db.SoftDeleteBook(database.Write(t.Context()), 1, admin.ID); err != nil {
	...
	go func() {
		_, err := s.purgeTrashedBooks(context.Background(), []int64{1})
		done <- err
	}()
	...
	var exists bool
	if err := database.Read(t.Context()).QueryRow("SELECT EXISTS(SELECT 1 FROM books WHERE id = 1)").Scan(&exists); err != nil {
		releaseOtherMutation()
...
func TestPurgeTreatsMissingAssetAsOrdinaryDrift(t *testing.T) {
	...
	var exists bool
	if err := database.Read(req.Context()).QueryRow("SELECT EXISTS(SELECT 1 FROM books WHERE id = 1)").Scan(&exists); err != nil {
		t.Fatalf("query book: %v", err)
	}
	if exists {
...
func listBookIDs(t *testing.T, w *httptest.ResponseRecorder) []int64 {
	t.Helper()
