...
func startActivity(t *testing.T, database *DB, user, asset int64, n int, now time.Time) ReadingActivityResult {
	t.Helper()
...
func checkpointActivity(t *testing.T, database *DB, user, asset int64, n int, origin time.Time, elapsed, action time.Duration, finished bool) ReadingActivityResult {
	t.Helper()
...
func TestReadingActivityRetriesAndTakeover(t *testing.T) {
	...
	startActivity(t, database, user, 1, 1, start)
	for i, elapsed := range []time.Duration{60 * time.Second, 60 * time.Second, 30 * time.Second} {
		...
		got, err := database.CheckpointWebReadingSession(context.Background(), user, 1, activityID(1),
	...
	startActivity(t, database, user, 2, 2, second)
	got := checkpointActivity(t, database, user, 1, 1, start, 120*time.Second, 0, true)
	if got.Active || got.CountedMS != 90_000 {
		t.Fatalf("late final = %+v", got)
	}
	if startActivity(t, database, user, 1, 1, start.Add(130*time.Second)).Active {
		t.Fatal("retry reclaimed retired session")
	}
	checkpointActivity(t, database, user, 2, 2, second, 60*time.Second, 0, false)
...
func TestReadingActivityIdleAndResume(t *testing.T) {
	...
	start := time.Now()
	startActivity(t, database, user, 1, 1, start)
	checkpointActivity(t, database, user, 1, 1, start, time.Minute, 0, false)
	got := checkpointActivity(t, database, user, 1, 1, start, time.Hour, time.Hour, false)
	if got.Active || got.CountedMS != ReadingIdleLimit.Milliseconds() {
		t.Fatalf("idle gap counted: %+v", got)
	}
	got = checkpointActivity(t, database, user, 1, 1, start, 2*time.Hour, 2*time.Hour, false)
	if got.Active || got.CountedMS != ReadingIdleLimit.Milliseconds() {
		t.Fatalf("retired interval revived: %+v", got)
	}
	startActivity(t, database, user, 1, 2, start.Add(2*time.Hour))
	got = checkpointActivity(t, database, user, 1, 2, start.Add(2*time.Hour), time.Minute, 0, false)
...
func TestReadingActivityActionBeforeIdleDeadline(t *testing.T) {
	...
	start := time.Now()
	startActivity(t, database, user, 1, 1, start)
	got := checkpointActivity(t, database, user, 1, 1, start, 310*time.Second, 290*time.Second, false)
...
func TestReadingActivityShortPauseAndLateCheckpoints(t *testing.T) {
	...
	startActivity(t, database, user, 1, 1, start)
	checkpointActivity(t, database, user, 1, 1, start, 30*time.Second, 0, false)
	resume := start.Add(90 * time.Second)
	got, err := database.StartWebReadingSession(ctx, user, 1, activityID(1), 1, resume)
	...
	for range 2 {
		got, err = database.CheckpointWebReadingSession(ctx, user, 1, activityID(1), checkpoint, resume.Add(30*time.Second))
	...
	got, err = database.CheckpointWebReadingSession(ctx, user, 1, activityID(1), ReadingActivityCheckpoint{
		ElapsedMS: 45_000, Finished: true,
	}, resume.Add(time.Minute))
	if err != nil || got.Active || got.CountedMS != 60_000 {
		t.Fatalf("old segment's late final = %+v, %v", got, err)
	}
	if got := startActivity(t, database, user, 1, 1, resume.Add(time.Minute)); got.Active {
		t.Fatal("old segment reclaimed current accounting")
	}
	got, err = database.StartWebReadingSession(ctx, user, 1, activityID(1), 1, resume.Add(time.Minute))
	if err != nil || !got.Active || got.CountedMS != 60_000 {
	...
	got, err = database.StartWebReadingSession(ctx, user, 1, activityID(1), 2, resume.Add(time.Hour))
	if err != nil || got.Active || got.CountedMS != 60_000 {
...
func TestReadingActivityMidnightAndTimeZoneChange(t *testing.T) {
	...
	startActivity(t, database, user, 1, 1, start)
	checkpointActivity(t, database, user, 1, 1, start, time.Minute, 0, true)
	days := readingTimeByDay(t, database, user)
	...
	startActivity(t, database, user, 1, 2, second)
	checkpointActivity(t, database, user, 1, 2, second, time.Minute, 0, true)
	days = readingTimeByDay(t, database, user)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.date, func(t *testing.T) {
			...
			for offset := 4 * time.Minute; offset <= elapsed; offset += 4 * time.Minute {
				checkpointActivity(t, database, user, 1, 1, start, offset, offset, offset == elapsed)
			}
			days := readingTimeByDay(t, database, user)
...
func TestReadingActivityIdentityAndElapsedValidation(t *testing.T) {
	...
	start := time.Now()
	startActivity(t, database, user, 1, 1, start)
	if _, err := database.StartWebReadingSession(context.Background(), user, 2, activityID(1), 0, start); !errors.Is(err, ErrInvalidReaderInput) {
	...
	for _, cp := range []ReadingActivityCheckpoint{{ElapsedMS: -1}, {ElapsedMS: 1, LastActivityMS: 2}, {LastActivityMS: -1}} {
		if _, err := database.CheckpointWebReadingSession(context.Background(), user, 1, activityID(1), cp, start); !errors.Is(err, ErrInvalidReaderInput) {
			t.Fatalf("invalid checkpoint accepted: %+v, %v", cp, err)
		}
	}
	got, err := database.CheckpointWebReadingSession(context.Background(), user, 1, activityID(1), ReadingActivityCheckpoint{ElapsedMS: 1 << 62, LastActivityMS: 1 << 62}, start.Add(time.Second))
