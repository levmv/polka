...
func TestWritebackMode(t *testing.T) {
	...
	if mode, err := OpenMode(database.Read(t.Context())); err != nil || mode != ModeManual {
		t.Fatalf("OpenMode default = %q, %v; want manual", mode, err)
	}

	for _, want := range []Mode{ModeOff, ModeManual, ModeAuto} {
		if err := SaveMode(database.Write(t.Context()), want); err != nil {
			t.Fatalf("SaveMode(%q): %v", want, err)
		}
		if got, err := OpenMode(database.Read(t.Context())); err != nil || got != want {
			t.Fatalf("OpenMode after save = %q, %v; want %q", got, err, want)
		}
	}

	if err := SaveMode(database.Write(t.Context()), "surprise"); !errors.Is(err, ErrInvalidMode) {
		t.Fatalf("SaveMode(surprise) error = %v; want ErrInvalidMode", err)
	}
	if got, _ := OpenMode(database.Read(t.Context())); got != ModeAuto {
